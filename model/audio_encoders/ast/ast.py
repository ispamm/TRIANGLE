...
class AudioEmbeddings(nn.Module):
    ...
    def forward(self, audio_spectrotriangles):  ### shape Bxn_sample_128x512
        
        audio_spectrotriangles = self.first_conv(audio_spectrotriangles.unsqueeze(1))
        b,c,_,_=audio_spectrotriangles.shape
        audio_tokens = audio_spectrotriangles.permute(0,2,3,1).reshape(b,-1,c)        
        cls_token = self.cls_token.expand(b,-1,-1)
        audio_tokens = torch.cat((cls_token,audio_tokens),dim=1)
        audio_pos_ids = list(range(self.token_length_per_frame + 1))
        audio_pos_ids = torch.tensor(audio_pos_ids, dtype=torch.long, device=audio_spectrotriangles.device).unsqueeze(0)
